...
def find_maximal_subarray_sum(nums: List[int], k: int) -> int:
    ...
    z = 0#пустая переменная, необходимая для счёта
    sum_max = 0#пустая переменная, в которую будем записывать значение максимальной суммы

    while z <= len(nums) - k:#z меньше либо равно длине последовательности минус длина исследуемых подпоследвательностей
        try:
            t=z
            sum_temp = 0#пустая переменная, необходимая для временной суммы
            while t < z+k:
                sum_temp += nums[t]
